...
import { DISK_WATERMARKS_DEFAULT, NO_VOLUME_USAGE, type DiskWatermarks } from '../store/volume-usage.ts';
...
/**
 * The composition root's published build entry (S35, the tool half of the
 * consumer-extension seam — `20-contract.md` § Tool registry extension,
 * mirroring § Console view registration's shape for the view half). It never
 * imports the compiler (invariant B8, enforced by
 * `scripts/check-no-compiler-in-runtime.ts`): it wires the lifecycle module,
 ...
 *
 * `src/server.ts` is the base's own consumption of this entry, calling
 * `composeAndStart()` with no extras — the same relationship
 * `console/src/main.tsx` has to `createConsole()`. A consumer's own
 * composition root calls this with `extraToolDeclarations`,
 * `extraModuleHandlers` and `extraRecoveryDescriptors` naming its own tools,
 * and its own `buildDir`/`consoleDir` pointing at artifacts *its* build
 * produced — so the derived image compiles and boots one registry and one
 * console fingerprint over both the base's set and its own, never two.
 */

const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', '..');

/** What a consumer's own composition root supplies to extend the base. Every field is additive — there is no way to remove or replace a base tool from here, which is what keeps the base's own set unchanged by any extension (S35.7's parity concern). */
export interface ComposeOptions {
  /** Defaults to `<repo root>/build` — a consumer building its own registry artifact under a different path passes it here rather than relying on the base's own layout. */
  readonly buildDir?: string;
  /** Defaults to `<repo root>/console/dist` — the console workspace's Vite build output (S18), verified at boot the same way `buildDir` is. A consumer's own console build (S19) lives elsewhere. */
  readonly consoleDir?: string;
  /** Declarations a consumer adds on top of `PRODUCTION_TOOL_DECLARATIONS`. Compiled together into one registry and one fingerprint (S35.3) by the consumer's own `build-registry`-equivalent — this function does not compile anything itself (B8: the compiler stays out of every runtime image, base or derived). */
  readonly extraToolDeclarations?: readonly ToolDeclaration[];
  /** Module-adapter registrations for each extra declaration's `ExecutionTarget`. Registered after every base target, so a name collision with a base target surfaces as `duplicate-registration` rather than silently shadowing it. */
  readonly extraModuleHandlers?: readonly { readonly target: ModuleTargetName; readonly handler: ModuleHandler }[];
  /** Recovery descriptors for each extra mutating declaration. Registered into the same catalogue as the base's own, so a duplicate `tool` name is caught the same fatal way at composition time. */
...
/**
 * `DeploymentConfig.watermarks` (`20-contract.md` § Deployment configuration)
 * — contract defaults 85 / 95 (`DISK_WATERMARKS_DEFAULT`). Resolved once
 * here and passed to `CloneStore`, `DispatchPipeline` and `Lifecycle` alike,
 * so the three cannot independently default and silently disagree about
 * where the watermark is.
 */
function resolveWatermarks(): DiskWatermarks {
  const rawMaintenance = process.env.DISK_WATERMARK_MAINTENANCE_PERCENT;
  const rawRefuse = process.env.DISK_WATERMARK_REFUSE_PERCENT;
  const maintenanceAtPercent = rawMaintenance === undefined || rawMaintenance.trim().length === 0 ? DISK_WATERMARKS_DEFAULT.maintenanceAtPercent : Number(rawMaintenance);
  const refuseAtPercent = rawRefuse === undefined || rawRefuse.trim().length === 0 ? DISK_WATERMARKS_DEFAULT.refuseAtPercent : Number(rawRefuse);
  if (!Number.isFinite(maintenanceAtPercent) || maintenanceAtPercent <= 0 || maintenanceAtPercent >= 100) {
    console.error(`server: DISK_WATERMARK_MAINTENANCE_PERCENT must be a number between 0 and 100 (got '${rawMaintenance}')`);
    process.exit(1);
  }
  if (!Number.isFinite(refuseAtPercent) || refuseAtPercent <= 0 || refuseAtPercent >= 100) {
    console.error(`server: DISK_WATERMARK_REFUSE_PERCENT must be a number between 0 and 100 (got '${rawRefuse}')`);
    process.exit(1);
  }
  if (refuseAtPercent <= maintenanceAtPercent) {
    console.error(`server: DISK_WATERMARK_REFUSE_PERCENT (${refuseAtPercent}) must be greater than DISK_WATERMARK_MAINTENANCE_PERCENT (${maintenanceAtPercent})`);
    process.exit(1);
  }
  return { maintenanceAtPercent, refuseAtPercent };
}
...
export async function composeAndStart(options: ComposeOptions = {}): Promise<void> {
  const buildDir = options.buildDir ?? path.join(repoRoot, 'build');
  const consoleDir = options.consoleDir ?? path.join(repoRoot, 'console', 'dist');
  const extraToolDeclarations = options.extraToolDeclarations ?? [];
  const extraModuleHandlers = options.extraModuleHandlers ?? [];
  const extraRecoveryDescriptors = options.extraRecoveryDescriptors ?? [];
  const toolDeclarations: readonly ToolDeclaration[] = [...PRODUCTION_TOOL_DECLARATIONS, ...extraToolDeclarations];
  ...
  const declarations = createDeclarations({
    ...
    remoteHostAllowlist,
    ceiling,
    registryEntry: (tool) => toolDeclarations.find((entry) => entry.name === tool) ?? null,
  ...
  const contractCapabilitySet = new Set(toolDeclarations.flatMap((e) => e.capabilities)) as unknown as ContractCapabilitySet;
  ...
  for (const descriptor of [
    ...
    // S35 — a consumer's own descriptors, registered last so a name collision
    // with a base descriptor surfaces as the same fatal `duplicate-registration`
    // every other collision here does, rather than silently shadowing it.
    ...extraRecoveryDescriptors,
  ]) {
  ...
  // S35 — a consumer's own module handlers, registered last so a target name
  // colliding with a base target is refused by `moduleAdapter.register`'s own
  // `duplicate-registration` rather than silently shadowing it.
  for (const { target, handler } of extraModuleHandlers) {
    const registered = moduleAdapter.register(target, handler);
    if (!registered.ok) {
      console.error(`server: ${registered.error.summary}`);
      process.exit(1);
      return;
  ...
  const lifecycle = createLifecycle({
    ...
    registryEntries: toolDeclarations,
  ...
  // Disarmed by `shutdown` below, before it awaits `maintenanceInFlight` —
  // without this, a mutation's fire-and-forget `checkWatermarkAfterMutation`
  // (`dispatch-pipeline.ts`) can still call `triggerMaintenance` after that
  // wait has already resolved, starting a pass against store/audit/journal
  // handles `lifecycle.shutdown()` is in the middle of closing.
  let shuttingDown = false;
  // Every mutating dispatch checks the watermark and, while still above it,
  // requests another pass — with no cooldown this reruns a full retention
  // pass (plus one `notification_outbox` row) on every single mutation while
  // pressure persists and nothing is evictable (e.g. every clone `pinned`).
  // Bounded to at most once per `resolveMaintenanceIntervalSeconds()`, the
  // same cadence the schedule itself already runs at — a watermark crossing
  // gets the next pass sooner than the schedule alone would, never more
  // often than the schedule's own interval.
  let lastWatermarkTriggerAt = 0;
  const maintenanceIntervalSeconds = resolveMaintenanceIntervalSeconds();
  function triggerMaintenance(reason: MaintenanceReason): void {
    if (shuttingDown) return;
    if (maintenanceInFlight) return;
    if (reason === 'watermark') {
      const now = Date.now();
      if (now - lastWatermarkTriggerAt < maintenanceIntervalSeconds * 1000) return;
      lastWatermarkTriggerAt = now;
    }
  ...
  }, maintenanceIntervalSeconds * 1000);
  // Unreferenced, so a pending timer never keeps the process alive on its own.
  maintenanceTimer.unref();

  const shutdown = (signal: NodeJS.Signals): void => {
    ready = false;
    // Set before anything is awaited below: a post-mutation watermark check
    // still in flight from a request that finished just as shutdown began
    // must not start a fresh maintenance pass after `maintenanceInFlight`
    // has already been waited out — `triggerMaintenance` checks this first.
    shuttingDown = true;
