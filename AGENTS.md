...
**Resolve the tier from the stamp first, then the configuration, and only then the self-report.** Where `AGENTKIT_TIER` is set in the environment, it *is* the tier and nothing further is looked up — `~/.agent-kit/tools/Invoke-CodexCommand.ps1` sets it, alongside `AGENTKIT_MODEL`, `AGENTKIT_EFFORT` and `AGENTKIT_COMMAND`, from the same table that picks the profile, so a session launched through that script carries its tier as a fact rather than an inference. This exists because the configuration read below is unexecutable in the sessions that need it most: the `architect` profile is `sandbox_mode = "read-only"` scoped to the workspace, and `~/.codex/` is outside it, so a `/redteam` session cannot open the file it is told to resolve from and falls to the self-report, which stops — every time, on the one command whose routing already required the strongest model. Environment variables cross that boundary; configuration files do not. A stamp naming a tier no row below carries is not a tier — treat it as unreadable and fall through.
...
- **Do not report a cost you did not measure.** A model is not given its own token counts or elapsed time, so any figure it states about its own run is an estimate presented as a measurement. `~/.agent-kit/tools/Measure-Session.ps1` reads the real per-call usage from the session transcript, and runs as both a `SessionEnd` hook (the log) and a `UserPromptSubmit` hook (`-Watch`, a silent-until-threshold context-size warning). Use it, or say nothing. It measures **Claude Code sessions only** — Codex writes a different schema this has no reader for, and Copilot records no token usage at all. Under either, *say nothing* is the whole instruction.
...
- **No work lands directly on the default branch, ever — not even a doc or contract edit made outside a named slash command, and not even the work-mirror refresh.** Before the first edit of any change, create a fresh branch off the default branch if one isn't already checked out. This applies uniformly: there is no category of work light enough to commit straight to the default branch. A direct-push exception for the *derived design-state record* (`design/state/work/` and `design/state-index.md`, written only by `~/.agent-kit/tools/Update-WorkMirror.ps1` or `~/.agent-kit/tools/Update-DesignProjection.ps1`) existed here previously, on the reasoning that a pull request over deterministic, machine-checked output is review theatre. It is retired: three consecutive attempts (2026-09-02, twice on 2026-09-05) were each blocked before reaching a merge — GitHub's own branch ruleset twice, the session's tool-use classifier once — so the exception never once achieved what it was for, and every refresh fell back to a pull request anyway (`design/90-decisions.md`, 2026-09-05 entry). The work-mirror refresh now follows the ordinary branch-commit-push-PR delegation below, the same as any other change. The loop this was meant to break — a PR per refresh means a merge per refresh, which puts `/clean` back on the table, which hands back to `/track`, which refreshes the mirror again — is accepted rather than engineered around.
...
5. Regenerate projections — `~/.agent-kit/tools/Update-DesignProjection.ps1`, a real run, not `-DryRun`.
6. Run the checker — `~/.agent-kit/tools/Test-DesignState.ps1`.
