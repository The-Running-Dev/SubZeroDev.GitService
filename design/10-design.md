...
| `scheduler.manage` | declaration | create and cancel held operations |
| `scheduler.read` | declaration | list held operations. **Split from `scheduler.manage` on 2026-08-13** so a caller may see the queue without being able to change it; both expand from the same coarse `schedule` scope, so the split is invisible above the capability layer |
...
| **Lifecycle** (L1) | The boot sequence, the recovery ladder — resolving each unsettled entry's descriptor, classifying it through the journal, and resuming or parking it — and the order the maintenance pass drives each module's `runRetention` in. Receives every collaborator by injection. **A checked module, not part of the composition root** — these are ordering decisions with failure modes, not wiring. **Not the cadences**: it exposes a `runMaintenance` the composition root's timer calls, and the snapshot interval belongs to the structured store, which owns the copies — see the retention-owners table above and the composition root below. | Whatever it is handed. | Boot, recover-declaration, run-maintenance, shutdown. |
| **Git operations** (L2) | Every repository-generic git behaviour: status, log, branches, health, diff, stage, commit, restore-paths, push, and the seven protected-base invariants. | L1, and L0 for the contract types. | Domain functions returning `ToolResult`. |
...
| **Host adapter** (L2) | Pull requests, checks, merges, deploy monitoring; the per-credential request budget and backoff. One implementation, GitHub via `gh`. | Exec, journal, the recovery catalogue — and credential preparation and `RepositoryConfig` **by injection**, never as value imports, so it reaches neither the credentials module nor git operations directly. | A host-shaped interface a second implementation could satisfy. |
| **Scheduler** (L2) | Due-job selection, missed-tick policy, grant re-intersection. | Declarations, journal, the structured store — and the dispatch pipeline and authorization's `grantIsLive` **by injection**, never as a value import. It takes no notifier edge today; it acquires one only if #49 wires terminal-state notification here rather than into dispatch, which is the open question that issue holds. | A tick engine. |
...
- **Recovery needs to compare a journal entry against real git state**, and the two live in
  ...
  is that **journal owns the rule, clone store owns the observation, and lifecycle owns the
  ladder**: clone store re-derives branch, `HEAD`, upstream and the two digests; the journal's pure
  `classify` turns that record plus a descriptor into a verdict; and the lifecycle module — which
  already resolves the descriptor from the catalogue — is what acts on it. Neither the git-state
  interpretation nor the classification rule is duplicated, and no edge reverses. Without this
  split the obvious implementation gives journal a dependency on clone store and closes a cycle.
  **A third party acting on the verdict is what keeps the split cheap**: neither of the two modules
  that own half the answer has to depend on the other to use it.
...
| | Merge conflict | PR state | **Terminal.** No rebase tool exists and by design never will | `precondition` naming the branch and both heads | Branch and commits intact; notifier fires — specified, not yet held on this path (#49) |
| | Required check failed | Check status | Terminal for the operation | `precondition` | PR open, nothing merged; notifier fires — specified, not yet held on this path (#49) |
